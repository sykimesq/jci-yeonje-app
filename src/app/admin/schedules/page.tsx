  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-jci-800">일정 관리</h1>
        ...
        {Array.from({ length: 12 }, (_, i) => i + 1).map((month) => {
          ...
          return (
            <div key={month} className="bg-white rounded-xl border border-jci-border overflow-hidden">
              <div className="px-4 py-3 bg-gray-50 border-b border-jci-border font-bold text-jci-800 text-sm">
                {MONTHS[month - 1]}
              </div>
                ...
                <tbody className="divide-y divide-jci-border">
                  {ms.map((s) => (
                    <tr key={s.id} className="hover:bg-gray-50">
                      <td className="px-4 py-3 w-12 text-center font-bold text-jci-500">{s.day || '-'}</td>
