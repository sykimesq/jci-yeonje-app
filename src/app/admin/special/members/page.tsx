...
export default function AdminSpecialMembersPage() {
  ...
  async function loadMembers() {
    const { data: years } = await (supabase as any)
      .from('years').select('id').eq('is_current', true).single()
    if (!years) return
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-amber-800">특우회 회원 관리</h1>
      ...
      <div className="relative mb-4">
        ...
        <input type="text" placeholder="이름 검색..." value={search}
          ...
          className="w-full pl-10 pr-10 py-2.5 bg-white border border-jci-border rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-amber-300" />
        {search && (
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-jci-border bg-amber-50">
              ...
              <th className="text-left px-4 py-3 font-medium text-jci-muted hidden lg:table-cell">휴대전화</th>
...
function MemberFormModal({ member, onClose, onSaved }: { member: Member | null; onClose: () => void; onSaved: () => void }) {
  const [form, setForm] = useState({
    ...
    member_type: (member?.member_type || 'special') as any,
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const payload = {
      year_id: years.id, name: form.name, name_hanja: form.name_hanja,
      name_english: form.name_english, birth_date: form.birth_date,
      phone: form.phone, address: form.address,
      workplace: form.workplace, position_in_company: form.position_in_company,
      workplace_address: form.workplace_address, workplace_phone: form.workplace_phone,
      workplace_fax: form.workplace_fax, home_address: form.home_address,
      member_type: form.member_type as any,
      jc_roles: form.jc_roles.split('\n').filter(Boolean),
      jc_awards: form.jc_awards.split('\n').filter(Boolean),
      photo_url: form.photo_url, order_index: member?.order_index || 0,
    }
    if (member) {
      ...
      <div className="relative bg-white rounded-2xl max-w-lg w-full max-h-[85vh] overflow-y-auto shadow-xl" onClick={(e) => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit} className="p-4 space-y-4">
          ...
          <div>
            ...
            <PhotoUpload path={form.photo_url} onUpload={(url) => setForm({...form, photo_url: url})}
          ...
          <div>
            ...
            <select value={form.member_type} onChange={e => setForm({...form, member_type: e.target.value})}
              className="w-full px-3 py-2 border border-jci-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-amber-300">
              <option value="special">특우회원</option>
              <option value="honorary">명예회원</option>
              <option value="junior">준회원</option>
            </select>
          </div>
          <div>
