...
export default function AdminSpecialRegulationsPage() {
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-amber-800">특우회 정관 관리</h1>
      ...
      <div className="space-y-4">
        {Object.entries(grouped).map(([cat, items]) => (
          <div key={cat} className="bg-white rounded-xl border border-jci-border overflow-hidden">
            <div className="px-4 py-3 bg-amber-50 border-b border-jci-border font-bold text-amber-800 text-sm flex items-center justify-between">
              <span>{cat}</span>
      ...
      {showForm && (
        <RegulationFormModal regulation={editing} onClose={() => { setShowForm(false); setEditing(null) }}
        ...
        <form onSubmit={handleSubmit} className="p-4 space-y-4">
          <div>
            ...
            <input value={form.category} onChange={e => setForm({...form, category: e.target.value})} required
              placeholder="특우회 정관, 경조 규정, 포상 규정..."
          ...
          <div>
            ...
            <textarea value={form.content} onChange={e => setForm({...form, content: e.target.value})} rows={10}
              className="w-full px-3 py-2 border border-jci-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-amber-300 font-mono" />
          </div>
